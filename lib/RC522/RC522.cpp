 ...
 * @LastEditTime: 2025-03-13 14:44:55
...
{
    // 打开命名空间 Card_ID（读写模式）
    preferences.begin("Card_ID", false);
    ...
    // 获取已存储的卡号列表
    String storedCards = preferences.getString("cardList", "");
    ...
    else
    {
        ...
        // 存储更新后的卡号列表
        preferences.putString("cardList", storedCards);
        RC522_Load_IDs();
...
{
    // 打开命名空间 Card_ID（只读模式）
    preferences.begin("Card_ID", true);

    // 从Flash中读取所有卡号
    String storedCards = preferences.getString("cardList", "无数据");
...
void RC522_Load_IDs()
{
    ...
    preferences.begin("Card_ID", true);
    // 从Flash中读取所有卡号
    storedCards = preferences.getString("cardList", "无数据");
